...
get_eurostat <-
  function(id = "educ_iste") {
    ...
    dat2 <- separate_(dat, col = colnames(dat)[1],  # muutin
                       into = cnames1, 
                       sep = ",", convert = FALSE)
    ...
    dat3 <- gather_(dat2, cnames2, "value", names(dat2)[-c(1:length(cnames1))]) # muutin
    
    # remove flags
    dat3$value <- extract_numeric(dat3$value) # muutin
